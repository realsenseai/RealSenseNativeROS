...
class D555ReconstructionV3(Node):
    ...
    def _setup_subscriptions(self):
        ...
        self.create_subscription(Image, f'{prefix}_Depth', self._depth_cb, self.best_effort_qos)
        self.create_subscription(Image, f'{prefix}_Color', self._color_cb, self.best_effort_qos)
        self.create_subscription(CameraInfo, f'{prefix}_Depth/camera_info', self._depth_info_cb, self.best_effort_qos)
        self.create_subscription(CameraInfo, f'{prefix}_Color/camera_info', self._color_info_cb, self.best_effort_qos)
        self.create_subscription(Imu, f'{prefix}_Motion', self._imu_cb, self.best_effort_qos)
        self.create_subscription(TFMessage, f'{prefix}/tf_static', self._tf_cb, self.tf_static_qos)
        self.get_logger().info(f"Subscribed to {prefix}/tf_static with BEST_EFFORT + TRANSIENT_LOCAL")
    ...
    def _depth_cb(self, msg: Image):
        try:
            if msg.encoding == '16UC1':
                ...
                depth = (np.frombuffer(msg.data, dtype=np.float32).reshape((msg.height, msg.width)) * 1000).astype(np.uint16)
            else:
                return
    ...
    def _imu_cb(self, msg: Imu):
        self.imu_accel = np.array([msg.linear_acceleration.x, msg.linear_acceleration.y, msg.linear_acceleration.z])
        self.imu_gyro = np.array([msg.angular_velocity.x, msg.angular_velocity.y, msg.angular_velocity.z])
    ...
    def _process_frame(self):
        ...
        try:
            ...
            if self.color_frame is not None:
                ...
                rgb_colors = self.color_frame[cv, cu].astype(np.float32) / 255.0
            ...
            depth_colors_bgr = cv2.applyColorMap(z_uint8.reshape(-1, 1), cv2.COLORMAP_TURBO).reshape(-1, 3)
            depth_colors = depth_colors_bgr[:, ::-1].astype(np.float32) / 255.0  # BGR to RGB
    ...
    def stop_recording(self):
        ...
        if self.recorded_frames:
            ...
            self.get_logger().info(f"Saved {len(self.recorded_frames)} frames to {filename}")
    ...
    def save_reconstruction(self):
        ...
        all_colors = np.vstack(self.accumulated_colors) if self.accumulated_colors else None
...
class ProGUI:
    ...
    def _render_3d_pointcloud(self, width: int, height: int, points: np.ndarray, 
        ...
        for i in range(max_points):
            cv2.circle(img, (u[i], v[i]), 1, (int(c[i, 2]), int(c[i, 1]), int(c[i, 0])), -1)
    ...
    def _draw_header(self, img: np.ndarray, title: str, width: int):
        ...
        cv2.putText(img, title, (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 0), 1)
    ...
    def _render_info_panel(self, width: int, height: int) -> np.ndarray:
        ...
        # Depth
        cv2.putText(img, "DEPTH CAMERA", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.45, self.ACCENT, 1)
        y += lh + 2
        ...
        for line in lines:
            cv2.putText(img, line, (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.38, self.TEXT, 1)
            y += lh - 2
        
        y += 8
        
        # Color
        cv2.putText(img, "COLOR CAMERA", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.45, self.ACCENT, 1)
        y += lh + 2
    ...
    def _render_metadata_panel(self, width: int, height: int) -> np.ndarray:
        ...
        for stream, data in list(self.node.metadata.items())[:4]:
            cv2.putText(img, stream.upper(), (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, self.ACCENT, 1)
            y += lh
            
            fields = ['Frame Counter', 'Actual Exposure', 'Gain Level', 'ASIC Temperature']
            for f in fields:
    ...
    def _render_tf_panel(self, width: int, height: int) -> np.ndarray:
        ...
        for name, tf in list(self.node.transforms.items())[:6]:
            short = name.replace('camera_', '').replace('_optical_frame', '').replace('_frame', '')
            t = tf.translation
            cv2.putText(img, f"{short}:", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.38, self.ACCENT, 1)
    ...
    def _render_imu_panel(self, width: int, height: int) -> np.ndarray:
        ...
        cv2.putText(img, "ACCELEROMETER (m/s2)", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, self.ACCENT, 1)
        y += lh
        
        a = self.node.imu_accel
        cv2.putText(img, f"  X: {a[0]:+8.4f}", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.38, self.TEXT, 1)
        y += lh - 4
        cv2.putText(img, f"  Y: {a[1]:+8.4f}", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.38, self.TEXT, 1)
        y += lh - 4
        cv2.putText(img, f"  Z: {a[2]:+8.4f}", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.38, self.TEXT, 1)
        y += lh + 5
        
        cv2.putText(img, "GYROSCOPE (rad/s)", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, self.ACCENT, 1)
        y += lh
        
        g = self.node.imu_gyro
        cv2.putText(img, f"  X: {g[0]:+8.6f}", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.38, self.TEXT, 1)
        y += lh - 4
        cv2.putText(img, f"  Y: {g[1]:+8.6f}", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.38, self.TEXT, 1)
        y += lh - 4
        cv2.putText(img, f"  Z: {g[2]:+8.6f}", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.38, self.TEXT, 1)
    ...
    def _render_stats_panel(self, width: int, height: int) -> np.ndarray:
        ...
        imu_rate = self.node.stats['imu_samples'] / elapsed if elapsed > 0 else 0
        ...
        for s in stats:
            if s:
                cv2.putText(img, s, (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.38, self.TEXT, 1)
            y += lh - 3
        ...
        # Controls
        cv2.putText(img, "CONTROLS:", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, self.ACCENT, 1)
        y += lh
        
        ctrls = ["R=Record B=Build", "C=Clear S=Save", "SPACE=Pause Q=Quit", "+/-=Zoom 1/2=Speed"]
        for c in ctrls:
            cv2.putText(img, c, (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.32, (150, 150, 150), 1)
            y += lh - 5
    ...
    def update(self) -> bool:
        ...
        if self.node.is_recording:
            ...
            cv2.putText(combined, "REC", (sx + 15, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        if self.node.is_reconstructing:
            cv2.circle(combined, (sx + 80, 15), 8, (0, 255, 0), -1)
            cv2.putText(combined, "BUILD", (sx + 95, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        if self.paused:
            cv2.putText(combined, "PAUSED", (sx + 180, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 255), 1)
