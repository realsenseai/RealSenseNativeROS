...
```bash
ros2 param set /D555_<serial> Depth.option.Enable_PointCloud 2
```
...
produced `sensor_msgs/msg/PointCloud2` samples with `x,y,z,rgb` fields. Keep one
long-lived PointCloud subscriber and keep Depth + Color readers active for the
most stable XYZRGB path. The host `/camera/points2` topic remains available when
you want a realsense-ros-compatible topic generated on the host.
